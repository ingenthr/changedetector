function httpopcounts(doc, meta) {
    ...
    //var s3re = new RegExp("^(\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] ([\d.]+) (\S+) (\S+) (\S+) (\S+) \"(.+?)\" (\d{3}) (\S+) (\S+) (\d+) (\d+) (\S+) \"(.+?)\" \"(.+?)\" (\S+)$/");

    //var s3re = new RegExp("^(\S+) (\S+) \\[([\w:/]+\s[+\-]\d{4})\\] ([\d.]+) (\S+) (\S+) (\S+) (\S+) \"(.+?)\" (\d{3}) (\S+) (\S+) (\d+) (\d+) (\S+) \"(.+?)\" \"(.+?)\" (\S+)$/");

    //var s3re = /(\S+) (\S+) \[([\w:\/]+\s[+\-]\d{4})\] ([\d.]+) (\S+) (\S+) (\S+) (\S+) "(.+?)" \d{3} (\S+) (\S+) (\d+) (\d+) (\S+) "(.+?)" "(.+?)" (\S+)/
    //var s3re = /^(\S+) (\S+) \[([\w:\/]+\s[+\-]\d{4})\] ([\d.]+) (\S+) (\S+) (\S+) (\S+) "(.+?)" (\d{3}) (\S+) (\S+) (\d+) (\d+) (\S+) "(.+?)" "(.+?)" (\S+)$/
...
//for (var i = 0, l = database.length; i < l; ++i) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (tokens) {
                ...
                //emit("Xception-regexmatch", lines[i]);  // used for debugging
            }
            //var aline = "a16c2e626139cecd2269d310345ce1516d4c7606c4cb50cb8a2c5776a096548e packages.couchbase.com [31/Oct/2014:23:01:16 +0000] 54.218.132.227 - 56360F107B696ED6 REST.GET.OBJECT ubuntu/couchbase.key \"GET /ubuntu/couchbase.key HTTP/1.1\" 304 - - 3097 193 - \"-\" \"Chef Client/11.6.0 (ruby-1.9.3-p429; ohai-6.18.0; x86_64-linux; +http://opscode.com)\" -";
            //var results = s3re.exec(aline);
            //emit(results[8], results[12]);
//      var ltokens = s3re.exec(lines[i]);
            //    emit(ltokens[8],ltokens[12]);
        }
    }
}